...
# 미로 데이터: 사용자로부터 입력받은 파일에서 미로 데이터를 읽어옴
maze = []
try:
    filename = turtle.textinput("게임선택", "게임 번호 입력:")               # 사용자에게 게임 번호 입력을 요청하고 파일명을 생성
    with open(filename+'.txt', 'r') as f:                                   # 입력받은 파일명을 바탕으로 미로 데이터 파일을 읽음
        lines = f.readlines()                                               # 파일의 각 줄을 읽음
    for line in lines:                                                      # 각 줄을 공백으로 나누어 정수형 리스트로 변환하여 미로 리스트에 추가
        maze.append(list(map(int, line.strip().split())))
except FileNotFoundError:
    print("파일을 찾을 수 없습니다.")                                       # 파일이 존재하지 않을 경우 오류 메시지 출력
except ValueError:
    print("파일의 내용이 올바른지 확인하세요.")                              # 파일의 내용이 올바르지 않을 경우 오류 메시지 출력
    sys.exit()                                                              # 프로그램 종료
